...
def get_db_connection():
    ...
    from modules.db import get_db_connection as _get_conn
    return _get_conn()
...
def create_dependent_filters(provincia_sel: str) -> Tuple[str, str, str]:
    ...
    if provincia_sel != "Todas":
        municipios = cached_db_query(
            "SELECT DISTINCT municipio FROM datos_uis WHERE provincia = %s AND municipio IS NOT NULL ORDER BY municipio",
            provincia_sel
        )["municipio"].tolist()
        ...
        if municipio_sel != "Todos":
            poblaciones = cached_db_query(
                "SELECT DISTINCT poblacion FROM datos_uis WHERE provincia = %s AND municipio = %s AND poblacion IS NOT NULL ORDER BY poblacion",
