...
def obtener_municipios(provincia: str) -> List[str]:
    ...
    try:
        df = pd.read_sql(
            "SELECT DISTINCT municipio FROM datos_uis WHERE provincia = %s ORDER BY municipio",
            conn,
            params=(provincia,),
        )
...
def obtener_poblaciones(provincia: str, municipio: str) -> List[str]:
    ...
    try:
        df = pd.read_sql(
            "SELECT DISTINCT poblacion FROM datos_uis WHERE provincia = %s AND municipio = %s ORDER BY poblacion",
            conn,
...
def guardar_en_base_de_datos_vip(
    ...
    try:
        ...
        cursor.execute("SELECT comercial FROM comercial_rafa WHERE apartment_id = %s", (apartment_id,))
        row = cursor.fetchone()
        ...
        if row:
            ...
            cursor.execute(
                ...
                FROM datos_uis WHERE apartment_id = %s
                """,
                (apartment_id,),
            )
            row = cursor.fetchone()
    ...
    with st.form(key="form_precontrato_standalone"):
        ...
        if submit:
            ...
            if not token_valido:
                ...
                cursor.execute(
                    "INSERT INTO precontrato_links (precontrato_id, token, expiracion, usado) VALUES (%s, %s, %s, false)",
...
def _listado_precontratos():
    ...
    cursor.execute("""
        SELECT 
            ...
            EXISTS(SELECT 1 FROM precontrato_links WHERE precontrato_id = p.id AND usado = true) as usado,
            p.mail, 
            p.permanencia, 
            p.telefono1, 
            p.telefono2
